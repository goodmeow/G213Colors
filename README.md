...
The GUI uses Iced's `tiny-skia` renderer with Iced/winit X11 windowing. It runs
natively on X11 sessions and through XWayland on Wayland sessions.
...
```bash
# Arch Linux
sudo pacman -S rust cargo pkgconf libusb libxkbcommon libx11 libxcb libxcursor libxi fontconfig

# Debian/Ubuntu
sudo apt-get install rustc cargo build-essential pkg-config libusb-1.0-0 libusb-1.0-0-dev libxkbcommon-dev libx11-dev libxcb1-dev libxcursor-dev libxi-dev libfontconfig1-dev
...
On Wayland desktops, the GUI requires XWayland because this build intentionally
uses Iced's X11 backend.
...
`cargo audit` may report `RUSTSEC-2026-0192` for `ttf-parser`. This is an
informational unmaintained warning from Iced's transitive font stack
(`iced -> cosmic-text -> fontdb -> ttf-parser`), not a direct dependency of this
application. The advisory has no patched `ttf-parser` release; recheck this when
Iced, `cosmic-text`, or `fontdb` release a compatible update.
